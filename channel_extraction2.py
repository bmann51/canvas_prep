...
class ConfigurableChannelExtractor:
    ...
    def find_er_images(self, source_base: str) -> List[str]:
        """Find all .er directories in the source path"""
        source_path = Path(source_base)
        er_images = []
        
        for item in source_path.iterdir():
            if item.is_dir() and item.name.endswith('.er'):
                er_images.append(item.name)
        
        return sorted(er_images)
...
def main():
    ...
    # Load from command line arguments
        ...
        # Auto-discover .er images if requested
        if args.all_er:
            temp_extractor = ConfigurableChannelExtractor(
                source_base=args.source,
                target_base="temp", 
                target_channels=["temp"],
                images=[]
            )
            discovered_images = temp_extractor.find_er_images(args.source)
            print(f"Auto-discovered {len(discovered_images)} .er images")
            images_to_process = discovered_images
        else:
        ...
        print("Required: --source, --target, --channels, and either --images or --all-er")
